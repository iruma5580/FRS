    ...
    if ($action === 'update') {
        ...
        if (!$id || !$code || !$name || !$category || !$location) {
            ...
            if ($count > 0) {
                ...
                $date_finish_val = $date_finish ?: null;
                $work_done_val = $work_done ?: null;
                $stmt = $conn->prepare("UPDATE assets SET asset_code=?, asset_name=?, category=?, location_name=?, status=?, assigned_user=?, notes=?, assigned_person_to_fix=?, due_date=?, work_order_number=?, priority_status=?, date_finish=?, work_done=?, work_done_status=? WHERE id=?");
                $stmt->bind_param("ssssssssssssssi", $code, $name, $category, $location, $status, $assigned_user, $notes, $assigned_person_to_fix, $due_date_val, $work_order_number, $priority_status, $date_finish_val, $work_done_val, $work_done_status, $id);
                if ($stmt->execute()) {
                    $qrImagePath = generateQrImage($code);
                    if ($qrImagePath !== null) {
                        $stmt2 = $conn->prepare("UPDATE assets SET qr_image=? WHERE id=?");
...
// $assets = [];
// $res = $conn->query("SELECT * FROM assets WHERE work_order_number IS NOT NULL AND priority_status IS NOT NULL ");
// $res = $conn->query("SELECT * FROM assets WHERE work_order_number IS NOT NULL AND work_order_number != '' AND priority_status IS NOT NULL AND priority_status != ''");
// if ($res) {
...
if ($isAdmin) {
    // Admin sees all assets with the filters on work_order_number and priority_status
...
// $assets now contains either all filtered assets (admin) or filtered assigned assets (non-admin)
