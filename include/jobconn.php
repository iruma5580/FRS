...
if ($search !== '') {
    ...
    status LIKE ? OR assigned_user LIKE ? OR notes LIKE ? OR assigned_person_to_fix LIKE ? OR work_order_number LIKE ? OR
    work_done_status LIKE ?  
    ";
    $likeSearch = "%$search%";
    $params = [$likeSearch, $likeSearch, $likeSearch, $likeSearch, $likeSearch, $likeSearch, $likeSearch, $likeSearch, $likeSearch, $likeSearch];
    ...
    if ($action === 'update') {
        ...
        if ($id) {
            ...
            if ($count > 0) {
                ...
                $stmt->bind_param("si", $work_done_status, $id);
                if ($stmt->execute()) {
                    $qrImagePath = generateQrImage($code);
                    if ($qrImagePath !== null) {
                        $stmt2 = $conn->prepare("UPDATE assets SET qr_image=? WHERE id=?");
                        ...
                        header("Location: job_orders.php?id=$id&updated=1");
                        exit;
                } else {
...
// $assets = [];
// // $res = $conn->query("SELECT * FROM assets WHERE work_order_number IS NOT NULL AND date_finish IS NULL ");
